...
module.exports = async ({ requester, method, url, body, propertiesMapping }) => {
  ...
  if (propertiesMapping?.pagination) {
    for (let i = responses[0][propertiesMapping.pagination.currentPage] + 1; i <= responses[0][propertiesMapping.pagination.totalPages]; i++) {
  ...
  if (propertiesMapping?.data) {
    return responses.flatMap(e => e[propertiesMapping.data])
  }
  return responses[0]
}
